...
class Categories_model extends CI_Model {
    ...
    public function renderTableRows() {
        ...
        foreach($result as $rs) {
            ...
            $rstr .= '<tr><td>';
            $rstr .= '<div class="form-row"><div class="input-wrapper warning-text"><img src="/images/app/icons/warning.png" class="warning-icon"> <i>You haven\'t created any Floorplan Categories for your website yet. Start by clicking "Create New Category" on the right.</b></i><img src="/images/app/icons/help.png" class="help-icon help" data-subject="add-menus"></div></div>';   
            $rstr .= '</td></tr>';
        }
    ...
    public function create() {
        ...
        $rstr .= $this->load->view('appForms/floorplanCategoryForm', $data, true);
        $rstr .= '</div>';
    ...
    public function edit() {
        ...
        $SQL = "SELECT * FROM fdcms_floorplans_categories WHERE category_id = '".$id."' LIMIT 1";
    ...
    public function save() {
        ...
        if($id == 0) {          
            $this->db->insert('fdcms_floorplans_categories',$data);
			$return["id"] = $this->db->insert_id();
            $return["message"] = "The New Floorplan Category [ ".$data["category_name"]." ] was created successfully.";
        } else {
            // Saving an existing Page    
            $this->db->where('category_id',$id);
			$this->db->update('fdcms_floorplans_categories',$data);
            $return["id"] = $id;
    ...
    public function getCategoryName($id) {
        $SQL = "SELECT * FROM fdcms_floorplans_categories WHERE category_id = '".$id."'";
        $query = $this->db->query($SQL);
        $row = $query->row();
